...
# CUDA kernel source code for adaptive thresholding
cuda_kernel = r'''
extern "C" __global__ void adaptive_threshold(const unsigned char *input, unsigned char *output, int width, int height, int neighborhood_size) {
...
kernel(
    ...
    (image_gpu, output_gpu, width, height, neighborhood_size)
)
...
# Save output image
cv2.imwrite('output_image_cuda.png', output)

# Print execution time
print(f"Execution Time: {execution_time * 1000:.2f} ms")

print("Output image saved as output_image_cuda.png")
